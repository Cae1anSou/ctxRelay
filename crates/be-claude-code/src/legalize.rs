use ctxrelay_backend::{BackendError, LoweringReport};
...
///
/// 返回 `Result`:IR 类型系统允许 `Reasoning.caps.verifiable_signature == true`
/// (property test 也会生成这种取值),但目前没有任何代码路径能把它写成目标原生
/// thinking block——曾经这里直接 `panic!`,一旦真出现这种输入(比如未来某个
/// frontend 真的能拿到签名字节),会让整个 `ctxrelay import` 裸崩而不是给出一个
/// 可以被 CLI 顶层 `Err(message)` 分支正常处理的错误。
pub fn legalize(doc: &Document) -> ctxrelay_backend::Result<(Document, LoweringReport)> {
