...
fn run_verify_gets_a_real_response_from_claude() {
    ...
    let doc = Document {
        ...
        turns: vec![Turn {
            ...
            blocks: vec![Block::Text {
                content: "我们在聊 ctxRelay 项目的设计".to_string(),
            }],
            timestamp: None,
        }],
    };
    ...
    let manifest =
        commit(lowered, &dest, backend.target(), report, ir_digest).expect("commit should succeed");
    ...
    assert!(
        ...
        "expected a non-empty summary from claude"
    );
